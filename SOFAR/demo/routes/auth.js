...
router.post("/register", (req, res) => {
  ...
  db.exec(`SELECT email FROM users WHERE email = ?`, [email], (result, fields) => {
    ...
    if(result[0]){
      ...
      db.exec(`INSERT INTO users SET ?`,{name: name, email: email, password: hashedPassword, birthday: birthday}, (results, fields) =>{
...
function sendEmail(email,token) {
  ...
  async function send(token) {
    const result = await transporter.sendMail({
        ...
        to: email,
        subject: 'Hello World',
        html: '<p>You requested for reset password, kindly use this <a href="http://localhost:3000/member/reset-password/' + token + '">link</a> to reset your password</p>'
    });
